...
export const useGameStore = defineStore('game', () => {
  ...
  if (typeof document !== 'undefined') {
    ...
    const recentChallengeIds = ref([]);
    const winnerBurst = ref(0);
    const seedValue = ref(Date.now() % 100000);
    ...
    const removeRecent = (challengeId) => {
        if (!challengeId)
            return;
    ...
    const markWinners = (winnerIds) => {
        if (!winnerIds.length || !activeChallenge.value)
            return;
        const validWinnerIds = winnerIds.filter((id) => activePlayerIds.value.includes(id));
        if (!validWinnerIds.length)
            return;
        if (settings.keepScore) {
            players.value = players.value.map((player) => validWinnerIds.includes(player.id) ? { ...player, score: player.score + 1 } : player);
        }
    ...
    const togglePlayer = (id) => {
        players.value = players.value.map((player) => player.id === id ? { ...player, enabled: !player.enabled } : player);
    };
    const updatePlayer = (id, payload) => {
        players.value = players.value.map((player) => player.id === id ? { ...player, ...payload } : player);
    };
  ...
  const setDarkMode = (value) => {
    ...
    const importChallenges = (payload) => {
